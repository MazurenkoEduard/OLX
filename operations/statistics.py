...
#
#
# class Statistic(BaseOperation):
#     def statistic_excel(self, path, sheet_name, id):
...
#             sheet_name = self.window.sheet_input_2.text()
...
#                 return
#             ids = self.statistic_excel(path, sheet_name, id)
#             if not ids:
#                 return None
#
#             if not self.preload():
...
#                     if i == 0:
#                         self.hide_popup()
#                     inventory_button = self.session.wait(
#                         '//div[@data-cy="inventory-stats"]/button',
#                         timer=10,
#                         condition="click",
#                     )
#                     if inventory_button:
...
#                             (i + 1) / len(ids) * self.window.stat_bar.maximum(),
#                             self.window.stat_bar,
#                         )
